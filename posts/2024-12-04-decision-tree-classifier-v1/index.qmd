...
description: "A from scratch implementation of the decision tree classifier algorithm in R"
author:
  - name: Rashmin Chitale
    url: 
date: 05-07-2022
categories: [datascience, R] # self-defined categories
image: images/dtree-v1-listing.png
...
rpart(type ~ length + weight, data = salmon_fish, parms = list(split = "information"))
...
get_information_gain <- function(parent, l_child, r_child, mode = "entropy"){
  ...
  if(mode == "gini"){
    gain = 
      get_gini_impurity(parent[, ncol(parent)]) 
    - (weight_l*get_gini_impurity(l_child[, ncol(l_child)]) 
       + weight_r*get_gini_impurity(r_child[, ncol(r_child)]))
  } else {
...
```{r echo = F}
ggdraw() + 
  draw_image("images/prpd3.png", width = 0.5) + 
  draw_image("images/scratchd2.png", width = 0.5, x = 0.5)
```
